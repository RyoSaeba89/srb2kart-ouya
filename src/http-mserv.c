...
#ifdef __ANDROID__
	// Static libcurl+mbedTLS has no system CA store on Android; point it at the
	// Mozilla bundle we ship in the data dir (extracted next to the WADs).
	{
		const char *dd = getenv("SRB2WADDIR");
		if (dd && dd[0])
		{
			static char ca_path[1024];
			snprintf(ca_path, sizeof ca_path, "%s/cacert.pem", dd);
