...
#ifdef __ANDROID__
			// Tegra 3's Cg-based GLES compiler rejects SRB2's GLSL shaders.
			// Bail out (return false) so the engine clears gr_shadersavailable
			// and renders fixed-function through gl4es instead of I_Error'ing.
			fprintf(stderr, "GL: vertex shader %d failed to compile, falling back to fixed-function:\n%s\n", i, infoLog);
			free(infoLog);
...
#ifdef __ANDROID__
			fprintf(stderr, "GL: fragment shader %d failed to compile, falling back to fixed-function:\n%s\n", i, infoLog);
			free(infoLog);
			pglDeleteShader(gl_vertShader);
...
#ifdef __ANDROID__
		{
			fprintf(stderr, "GL: shader program %d failed to link, falling back to fixed-function\n", i);
			pglDeleteShader(gl_vertShader);
			pglDeleteShader(gl_fragShader);
			return false;
		}
...
{
#ifdef __ANDROID__
	fprintf(stderr, "VIDDBG SetModelView/glViewport: %dx%d\n", (int)w, (int)h);
#endif
...
#ifdef __ANDROID__
	// DIAG (run once): confirm gl4es is connected and actually rendering into the
	// buffer we read back. Red-clear then glReadPixels(0,0).
	{
		static int diag_done = 0;
		if (!diag_done)
		{
			unsigned char px[4] = {7, 7, 7, 7};
			diag_done = 1;
			pglClearColor(1.0f, 0.0f, 0.0f, 1.0f);
			pglClear(GL_COLOR_BUFFER_BIT);
			pglReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, px);
			fprintf(stderr, "VIDDBG GL: version=%s renderer=%s\n",
				(const char *)pglGetString(GL_VERSION), (const char *)pglGetString(GL_RENDERER));
			fprintf(stderr, "VIDDBG GL: red-clear readback = %d,%d,%d,%d (expect 255,0,0,255)\n",
				px[0], px[1], px[2], px[3]);
		}
	}
#endif
