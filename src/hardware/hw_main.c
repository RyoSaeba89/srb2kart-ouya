...
// --------------------------------------------------------------------------
// Upload all of the level's wall textures and flats to the GL driver up-front
// (at level load), instead of lazily on first sight. gl4es uploads + converts a
// texture on first bind, which causes a visible hitch the first time a new area
// scrolls into view on Tegra 3. Doing it here folds the cost into the loading
// screen. (R_PrecacheLevel skips all of this in 3D mode.)
// --------------------------------------------------------------------------
void HWR_PrecacheLevel(void)
{
	...
	// wall textures referenced by any sidedef (+ the sky)
	texpresent = calloc(numtextures, sizeof (*texpresent));
	if (texpresent)
	{
		for (i = 0; i < (size_t)numsides; i++)
		{
			if (sides[i].toptexture > 0 && sides[i].toptexture < numtextures)
				texpresent[sides[i].toptexture] = 1;
			if (sides[i].midtexture > 0 && sides[i].midtexture < numtextures)
				texpresent[sides[i].midtexture] = 1;
			if (sides[i].bottomtexture > 0 && sides[i].bottomtexture < numtextures)
				texpresent[sides[i].bottomtexture] = 1;
		}
		if (skytexture > 0 && skytexture < numtextures)
			texpresent[skytexture] = 1;
