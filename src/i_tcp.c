...
#if defined(__ANDROID__) && !defined(NONET)
// bionic API<21 has no out-of-line htons/htonl/ntohs/ntohl; <sys/endian.h>
// provides them as __builtin_bswap macros. The normal <netinet/in.h> chain
// doesn't pull it in on this target, so force it here for the whole TU
// (this file also #includes i_addrinfo.c, which relies on the same macros).
