...
#ifdef __ANDROID__
// Palette -> RGB565 lookup table, rebuilt in I_SetPalette. Lets the Android
// present path convert the 8bpp software framebuffer straight into the locked
// streaming texture in one pass (skips SDL_BlitSurface + a second UpdateTexture
// copy), which was the dominant per-frame cost once -O3 sped up the renderer.
static      Uint16       pal565[256];
#endif
#if 0
...
#ifndef __ANDROID__
			// On Android SDL_SetWindowSize maps to SurfaceHolder.setFixedSize and
			// desyncs SDL's window size from the real EGL surface (it reported
			// 320x200 while the surface was full-screen) -> glViewport rendered a
			// tiny corner. Leave the system-provided surface size alone.
...
#ifdef __ANDROID__
	// The Android GL/render surface size is dictated by the SurfaceView (and our
	// setFixedSize perf downscale), NOT by windowedModes. SRB2 picked e.g.
	// 1280x800 while the real EGL surface is 960x540, so the viewport/screen
	// texture only covered a sub-rect (game appeared in a corner). Sync vid to the
	// real surface so we fill the screen.
	//
	// For SOFTWARE on Tegra 3 the per-pixel CPU cost dominates, so we also render
	// at a *reduced* internal resolution and let SDL_RenderCopy upscale to the
	// surface (then the HWC upscales that to the display). The render height is
	// read from <HOME>/.srb2kart/renderheight.txt (default 360) so it can be
	// tuned without a rebuild; 0 = render at native surface size.
	if (window)
	{
		int ww = 0, wh = 0, gw = 0, gh = 0, sw, sh;
		SDL_GetWindowSize(window, &ww, &wh);
#ifdef HWRENDER
		SDL_GL_GetDrawableSize(window, &gw, &gh);
#endif
		// the real surface size (GL drawable in GL mode, else window size)
		sw = (rendermode == render_opengl && gw > 0) ? gw : ww;
		sh = (rendermode == render_opengl && gh > 0) ? gh : wh;

		if (sw > 0 && sh > 0)
		{
			int rw = sw, rh = sh; // render resolution

#ifdef HWRENDER
			if (rendermode != render_opengl)
#endif
			{
				// software: optional downscale of the render target.
				// 540 = validated default (sharp, ~45fps on Tegra 3); render
				// height can be overridden via renderheight.txt (lower = faster
				// but blurrier; the user found <480 too blurry to read).
				int targetH = 540;
				...
				if (home)
				{
					char p[1024];
					FILE *rf;
					snprintf(p, sizeof p, "%s/.srb2kart/renderheight.txt", home);
					rf = fopen(p, "r");
					if (rf)
					{
						int v = 0;
				...
				if (targetH > 0 && sh > targetH)
				{
					rh = targetH;
					rw = ((sw * targetH / sh) + 1) & ~1; // keep even, preserve aspect
				}
			}

			vid.width = width = rw;
			vid.height = height = rh;
			realwidth = (Uint16)rw;
			realheight = (Uint16)rh;
			fprintf(stderr, "VIDDBG SDLSetMode: surface=%dx%d render(vid)=%dx%d rendermode=%d\n",
				sw, sh, rw, rh, (int)rendermode);
			// VID_SetMode sets src_rect.w/h = vid.width/height after we return.
		}
	}
#endif
...
void I_FinishUpdate(void)
{
	...
	// --- OUYA perf instrumentation: split frame time into "present" (blit/
	// upload/swap, done here) vs "rest" (game logic + R_RenderPlayerView). ---
	{
		static Uint64 perf_freq = 0, perf_lastend = 0;
		static double perf_frame_acc = 0.0, perf_present_acc = 0.0, perf_frame_max = 0.0;
		static int perf_n = 0;
		Uint64 perf_t0, perf_t1;
		if (!perf_freq) perf_freq = SDL_GetPerformanceFrequency();
		perf_t0 = SDL_GetPerformanceCounter();
	...
	if (rendermode == render_soft && screens[0])
	{
		...
		{
#ifdef __ANDROID__
			// Fast path: convert the 8bpp framebuffer straight into the locked
			// RGB565 streaming texture (one pass) instead of BlitSurface->32/16
			// surface + a separate UpdateTexture copy. Falls back if the texture
			// isn't RGB565 (only happens with -softblit, unused on Android).
			void *texpx = NULL;
			int texpitch = 0;
			if (!usesdl2soft && SDL_LockTexture(texture, NULL, &texpx, &texpitch) == 0)
			{
				const Uint8 *srcrow = (const Uint8 *)bufSurface->pixels;
				int srcpitch = bufSurface->pitch;
				int w = vid.width, h = vid.height, x, y;
				for (y = 0; y < h; y++)
				{
					const Uint8 *s = srcrow + (size_t)y * srcpitch;
					Uint16 *d = (Uint16 *)((Uint8 *)texpx + (size_t)y * texpitch);
					for (x = 0; x < w; x++)
						d[x] = pal565[s[x]];
				}
	...
	else if (rendermode == render_opengl)
	{
		...
		perf_t1 = SDL_GetPerformanceCounter();
		if (perf_lastend)
		{
			{ double thisframe = (double)(perf_t1 - perf_lastend) * 1000.0 / perf_freq;
			  perf_frame_acc += thisframe;
			  if (thisframe > perf_frame_max) perf_frame_max = thisframe; }
			perf_present_acc += (double)(perf_t1 - perf_t0)      * 1000.0 / perf_freq;
			if (++perf_n >= 60)
			{
				double fms = perf_frame_acc / perf_n;
				double pms = perf_present_acc / perf_n;
				printf("PERF[%s] %.1f fps | frame %.1f ms (worst %.1f) = present %.1f ms + rest %.1f ms | %dx%d\n",
					(rendermode == render_opengl) ? "gl" : "soft",
					(fms > 0.0) ? 1000.0 / fms : 0.0, fms, perf_frame_max, pms, fms - pms,
					vid.width, vid.height);
				perf_frame_acc = perf_present_acc = 0.0;
				perf_frame_max = 0.0;
				perf_n = 0;
			}
		}
		perf_lastend = perf_t1;
	}
...
#ifdef __ANDROID__
		pal565[i] = (Uint16)(((palette[i].s.red   & 0xF8) << 8) |
		                     ((palette[i].s.green & 0xFC) << 3) |
		                     ((palette[i].s.blue  & 0xF8) >> 3));
#endif
	}
...
#ifdef __ANDROID__
		// Request a GLES2 context: gl4es translates the engine's desktop-GL
		// calls onto it. Must be set before the SDL_WINDOW_OPENGL window/context
		// is created so the EGL config matches.
...
void I_StartupGraphics(void)
{
	...
	if (rendermode == render_none)
	{
		...
		{
#if defined(HWRENDER)
			// GL (gl4es) is validated on Tegra and much faster than software
			// (GPU-accelerated, ~60fps vs ~28). Default to it; override via
			// renderer.txt ("software") or -software.
			rendermode = render_opengl;
			CONS_Printf("Android: defaulting to OpenGL (gl4es) renderer.\n");
#else
			rendermode = render_soft;
