...
#include <errno.h>
#include <sys/wait.h>
// NEWSIGNALHANDLER forks at startup (I_Fork) so the parent can monitor the
// child for crash signals. On Android this deadlocks: SDL has already spawned
// audio/event threads, so the fork()'d child inherits libc mutexes locked by
// threads that don't exist in the child -> it hangs in futex_wait, and the
// parent hangs in wait(). Fall back to the plain (non-forking) signal handler.
#ifndef __ANDROID__
