...
#ifdef __ANDROID__
#include "gl4esinit.h" // gl4es_GetProcAddress / set_getprocaddress / initialize_gl4es
#endif
...
{
#ifdef __ANDROID__
	// The renderer asks for desktop-GL entry points (glBegin-era + VBO/shaders);
	// gl4es supplies them and translates to GLES2. SDL_GL_GetProcAddress would
	// only return the (much smaller) native GLES symbol set.
...
{
#ifdef __ANDROID__
	// gl4es is statically linked. Point it at the live GLES2 driver (the SDL GL
	// context is already current here, see Impl_CreateWindow) and initialise it
	// before any desktop-GL call is resolved through GetGLFunc().
...
#ifdef __ANDROID__
	// gl4es on Tegra renders the MakeScreenFinalTexture/DrawScreenFinalTexture
	// capture+redraw path as solid white (the glCopyTexImage2D screen grab is
	// not reproduced correctly). The scene is already drawn to the backbuffer at
	// a full-surface viewport, so just present it directly.
