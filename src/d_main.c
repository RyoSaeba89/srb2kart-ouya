...
#ifdef __ANDROID__
// OUYA perf split: accumulate time spent in game logic (TryRunTics) vs the
// software/GL frame render (D_Display), printed periodically from D_SRB2Loop.
precise_t dbg_tic_accum = 0, dbg_disp_accum = 0;
int dbg_tic_started = 0, dbg_disp_n = 0;
#endif
...
void D_SRB2Loop(void)
{
	...
	for (;;)
	{
...
#ifdef __ANDROID__
			{
				extern precise_t dbg_tic_accum; extern int dbg_tic_started;
				precise_t _t = I_GetPreciseTime();
				TryRunTics(realtics);
				dbg_tic_accum += I_GetPreciseTime() - _t;
				dbg_tic_started = 1;
			}
#else
			TryRunTics(realtics);
		...
		{
#ifdef __ANDROID__
			extern precise_t dbg_disp_accum; extern int dbg_disp_n;
			precise_t _td = I_GetPreciseTime();
			D_Display();
			dbg_disp_accum += I_GetPreciseTime() - _td;
			if (++dbg_disp_n >= 60)
			{
				extern precise_t dbg_tic_accum; extern int dbg_tic_started;
				double prec = (double)I_GetPrecisePrecision();
				printf("PERF2: render(D_Display) %.1f ms | logic(tics) %.1f ms  [avg/frame over %d]\n",
					dbg_disp_accum * 1000.0 / prec / dbg_disp_n,
					dbg_tic_started ? (dbg_tic_accum * 1000.0 / prec / dbg_disp_n) : 0.0,
					dbg_disp_n);
				dbg_disp_accum = 0; dbg_tic_accum = 0; dbg_disp_n = 0;
			}
#else
			D_Display();
#endif
...
const char *D_Home(void)
{
	...
	// NOTE: the NDK CMake toolchain defines bare ANDROID, which used to hit a
	// hardcoded "/data/data/org.srb2/" here (wrong package + not writable by us)
	// -> I_Error "Couldn't write game config". Our Android glue (srb2k_android.c)
	// sets $HOME to the app's writable external files dir, so fall through to the
	// normal I_GetEnv("HOME") path below instead.
#ifdef _arch_dreamcast
